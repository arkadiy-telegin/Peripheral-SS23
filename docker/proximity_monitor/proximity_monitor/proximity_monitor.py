...
class BackCameraMode(Enum):
    HI_RES = True
    LOW_RES = False

HI_RES_PROFILE = '424x240x6'
LOW_RES_PROFILE = '320x240x6'
...
class ProximityMonitorNode(Node):
    def __init__(self):
        ...
        # Disable mode change for now.
        # self.camera_mode_subscriber = self.create_subscription(Bool,
        #                                                        f'/roboy/pinky/sensing/{self.camera_name}/mode',
        #                                                        self.change_camera_mode, 10)
    ...
    def change_camera_mode(self, mode_bool: Bool):
        mode = BackCameraMode(mode_bool.data)
        self.get_logger().info(f'Changing {self.camera_name} mode to {mode.name}')
        request = SetParameters.Request()
        request.parameters = [
            Parameter(name='rgb_camera.profile', value=LOW_RES_PROFILE if mode == BackCameraMode.LOW_RES else HI_RES_PROFILE).to_parameter_msg(),
        ]
        self.parameter_client.call(request)
